...
class ventas_articulos extends fbase_controller
{
     ...
     * los registros de artículos en facturas de cierto rango y/o con otros
     * filtros 
     */
    ...
    public function generar_consulta_filtros(){
        ...
        FROM lineasfacturascli t1 INNER JOIN facturascli t2 INNER JOIN articulos t3 ON t3.referencia = t1.referencia ".$familia."  AND t1.idfactura = t2.idfactura AND t2.fecha BETWEEN '".$fecha1."' AND '".$fecha2."' 
        ...
        $this->consulta_articulos = $this->db->select($sql);

        $sql = "SELECT SUM(t1.pvptotal) as total,  SUM(t1.cantidad) as cantidad
        FROM lineasfacturascli t1 INNER JOIN facturascli t2 INNER JOIN articulos t3 ON t3.referencia = t1.referencia ".$familia."  AND t1.idfactura = t2.idfactura AND t2.fecha BETWEEN '".$fecha1."' AND '".$fecha2."' 
        ".$prov_servicio." AND (lower(t1.descripcion) LIKE lower('%".$this->filtro_b."%') OR lower(t1.referencia) LIKE lower('%".$this->filtro_b."%')  ) ".$estado_factura." ORDER BY t2.fecha ASC ";

        $this->total_articulos = $this->db->select($sql)[0];
        $this->consolidado = $_POST["consolidado"];
