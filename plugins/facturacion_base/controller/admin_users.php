...
class admin_users extends fs_controller
{
    ...
    protected function private_core()
    {
        ...
        if (filter_input(INPUT_POST, 'nnick')) {
            $agente = new agente();
            $ag = $agente->get($_REQUEST["provasoc"]);
            if(!isset($_POST["yaesta"]))
                $ag = $agente->get($_REQUEST["id_proveedor"]);
            if(!$ag){
                $this->add_user();
                $this->proveedor_asociado();
            }else{
                if(!isset($_POST["yaesta"]))
                    $this->new_error_msg("El ID ".$_REQUEST["id_proveedor"]." Provedor ingresado ya existe");
                else
                    $this->new_error_msg("El ID ".$_REQUEST["provasoc"]." Provedor ingresado ya existe");
        ...
        private function add_user()
    {
        ...
        if ($nu) {
            ...
            if ($nu->set_password(filter_input(INPUT_POST, 'npassword'))) {
                ...
                $nu->user_admin = (bool) filter_input(INPUT_POST, 'usadmin');
                $nu->user_jefe_patio = (bool) filter_input(INPUT_POST, 'usjefe');
                
                if($nu->user_jefe_patio){
                    $nu->fs_page = "tpv_recambios";
                }
                ...
                if ($nu->save()) {
                    ...
                    if($nu->user_admin){
                        ...
                        $page = new fs_page();
                        $acc = new fs_access();
                        foreach ($page->all() as $pag){
                            
                            
                            $acc->fs_page = $pag->name;
                            $acc->fs_user = $nu->nick;
                            $acc->allow_delete = 1;
                            
                            $val =  strpos($pag->name, "admin");
                            if( $val === false){
                                $acc->save();
                            }
                            
                        }
                        $acc->fs_page = "admin_user";
                        $acc->fs_user = $nu->nick;
                        $acc->allow_delete = 1;
                        $acc->save();
                        $acc->fs_page = "admin_users";
                        $acc->fs_user = $nu->nick;
                        $acc->allow_delete = 1;
                        $acc->save();
                        $acc->fs_page = "admin_agentes";
                        $acc->fs_user = $nu->nick;
                        $acc->allow_delete = 1;
                        $acc->save();
                    ...
                    if($nu->user_jefe_patio){
                        ...
                        $acc->fs_page = "informes_lav";
                        $acc->fs_user = $nu->nick;
                        $acc->allow_delete = 0;
                        $acc->save();
                        $acc->fs_page = "tpv_recambios";
                        $acc->fs_user = $nu->nick;
                        $acc->allow_delete = 0;
                        $acc->save();
                        $acc->fs_page = "ventas_articulo";
                        $acc->fs_user = $nu->nick;
                        $acc->allow_delete = 0;
                        $acc->save();
                        $acc->fs_page = "ventas_articulos";
                        $acc->fs_user = $nu->nick;
                        $acc->allow_delete = 0;
                        $acc->save();
                        $acc->fs_page = "ventas_factura";
                        $acc->fs_user = $nu->nick;
                        $acc->allow_delete = 0;
                        $acc->save();
                        $acc->fs_page = "ventas_facturas";
                        $acc->fs_user = $nu->nick;
                        $acc->allow_delete = 0;
                        $acc->save();
                        $acc->fs_page = "facturas_lav";
                        $acc->fs_user = $nu->nick;
                        $acc->allow_delete = 0;
                        $acc->save();
                        $acc->fs_page = "servicios_lav";
                        $acc->fs_user = $nu->nick;
                        $acc->allow_delete = 0;
                        $acc->save();
