...
class informe_articulos extends fbase_controller
{
    ...
    protected function private_core()
    {
        ...
        if(isset($_REQUEST["familia_cons"])){
            $this->familia_cons = $_REQUEST["familia_cons"];
        }
        if(isset($_REQUEST["servicio_cons"])){
            $this->servicio_cons = $_REQUEST["servicio_cons"];
        }
    ...
    private function top_articulo_faccli()
    {
        ...
        if (!$toplist || isset($_POST['desde'])) {
            $toplist = array();
            $articulo = new articulo();
            $sql = "SELECT IF(a.servicio = 1, 'SERVICIO', 'ARTÍCULO' ) as servicio, fa.descripcion as familia, l.referencia, SUM(l.cantidad) as unidades, SUM(l.pvptotal/f.tasaconv) as total"
                . " FROM lineasfacturascli l, facturascli f, articulos a, familias fa "
                ...
                . " AND a.referencia = l.referencia  AND a.codfamilia LIKE '%".$this->familia_cons."%'  "
                . " AND  a.servicio LIKE '%".$this->servicio_cons."%'  AND fa.codfamilia = a.codfamilia "
                . " GROUP BY referencia"
