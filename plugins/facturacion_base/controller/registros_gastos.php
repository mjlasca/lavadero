<?php
/*
 * This file is part of registros_gastoscion_base
...
class registros_gastos extends fbase_controller
{
    ...
    {
        parent::__construct(__CLASS__, 'Registro Gastos', 'gastos');
    }
    ...
    protected function private_core()
    {
        ...
        if (isset($_POST['buscar_lineas'])) {
            ...
            $this->template = 'extension/registros_gastoss_articulo';
            ...
            if ($this->mostrar == 'sinpagar') {
                $this->resultados = $this->registros_gastos->all_sin_pagar($this->offset, FS_ITEM_LIMIT, $this->order . $order2);
                ...
                $this->resultados = $this->registros_gastos->all($this->offset, FS_ITEM_LIMIT, $this->order . $order2);
        }
    }
    ...
    public function buscar_lineas()
    {
        ...
        $this->template = 'ajax/compras_lineas_registros_gastoss';
    ...
    private function share_extension()
    {
        ...
        $extensiones = array(
            array(
                'name' => 'registros_gastoss_proveedor',
                'page_from' => __CLASS__,
                'page_to' => 'compras_proveedor',
                'type' => 'button',
                'text' => '<span class="glyphicon glyphicon-list" aria-hidden="true"></span> &nbsp; registros_gastoss',
                'params' => ''
            ),
            array(
                'name' => 'registros_gastoss_agente',
                'page_from' => __CLASS__,
                'page_to' => 'admin_agente',
                'type' => 'button',
                'text' => '<span class="glyphicon glyphicon-list" aria-hidden="true"></span> &nbsp; registros_gastoss de proveedor',
                'params' => ''
            ),
            array(
                'name' => 'registros_gastoss_articulo',
                'page_from' => __CLASS__,
    ...
    {
        return $this->fbase_sql_total('registro_gastos', 'idfactura', 'WHERE pagada = false');
    }

    private function total_registros()
    {
        return $this->fbase_sql_total('registro_gastos', 'idfactura');
    }
    ...
    {
        $fact = $this->registros_gastos->get($_GET['delete']);
        if ($fact) {
            /// obtenemos las líneas de la registros_gastos antes de eliminar
            ...
            if ($fact->delete()) {
                ...
                /// ¿Esta registros_gastos es rectificativa de otra?
                if ($fact->idfacturarect) {
                    $original = $this->registros_gastos->get($fact->idfacturarect);
                    if ($original) {
                ...
                $this->new_error_msg("¡Imposible eliminar la registros_gastos!");
            }
        } else {
            $this->new_error_msg("registros_gastos no encontrada.");
        }
    }
