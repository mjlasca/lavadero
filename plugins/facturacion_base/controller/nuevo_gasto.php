...
use FacturaScripts\model\gasto;
use FacturaScripts\model\linea_registro_gastos;
use FacturaScripts\model\metodo_pago;
use FacturaScripts\model\registro_gasto;

require_once 'plugins/facturacion_base/extras/fbase_controller.php';

class nuevo_gasto extends fbase_controller
{
    ...
    {
        parent::__construct(__CLASS__, 'Nuevo Gasto', 'gastos', FALSE, FALSE, TRUE);
    ...
    protected function private_core()
    {
        ...
        if (isset($_REQUEST['buscar_proveedor'])) {
			...
			$this->proveedores_insumo($_REQUEST['buscar_proveedor']);
        } else if (isset($_REQUEST['datosproveedor'])) {
            $this->datos_proveedor();
	} else if (isset($_REQUEST['referencia_combo'])) {
            //$this->get_combo_precio($_REQUEST['referencia_combo']);
     ...
     * @param string $query
     */
    protected function proveedores_insumo($query)
    {
    ...
    public function get_combo_precio($refi)
    {
		$this->template = FALSE;
				
        $sql = "SELECT preciocombo FROM articulos WHERE referencia = '".$refi."' ";
        $dat_combo = $this->db->select($sql);
		
        header('Content-Type: application/json');
        echo json_encode($dat_combo);
    }
    ...
    private function new_search()
    {
        ...
        foreach ($this->results as $i => $value) {
            ...
            /// si tenemos un codproveedor, hay que buscar el coste para este proveedor
    ...
    private function get_precios_articulo()
    {
        ...
        $this->template = 'ajax/nuevo_gasto_precios';
    ...
    {
        /// cambiamos la plantilla HTML
        $this->template = 'ajax/nuevo_gasto_combinaciones';
                    ...
                    if (isset($_POST['referencia_' . $i])) {
                        ...
                        if ($linea->save()) {
                            if ($articulo && isset($_POST['costemedio'])) {
                                ...
                                $this->actualizar_precio_proveedor($pedido->codproveedor, $linea, $i);
                        ...
                        if ($linea->save()) {
                            if ($articulo) {
                                ...
                                if (isset($_POST['costemedio'])) {
                                    $this->actualizar_precio_proveedor($albaran->codproveedor, $linea, $i);
                                }
                            }
    ...
    private function nueva_factura_proveedor()
    {
        ...
        if ($continuar) {
            ...
            if ($regularizacion->get_fecha_inside($factura->fecha)) {
                ...
                for ($i = 0; $i < $n; $i++) {
                    if (isset($_POST['referencia_' . $i])) {
                        $linea = new linea_registro_gastos();
                        ...
                        if ($linea->save()) {
                            if ($articulo) {
                                ...
                                if (isset($_POST['costemedio'])) {
                                    ...
                                    $this->actualizar_precio_proveedor($factura->codproveedor, $linea, $i);
                                }
                            }
     ...
     */
    private function actualizar_precio_proveedor($codproveedor, $linea, $i)
    {
        ...
        $imp0 = $this->impuesto->get_by_iva($_REQUEST["iva_".$i]);        
    ...
    private function search_from_proveedor()
    {
        ...
        $sql = "SELECT * FROM gastos WHERE bloqueado = false";
        $separador = ' AND';
