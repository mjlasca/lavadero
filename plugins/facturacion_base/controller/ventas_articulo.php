...
class ventas_articulo extends fbase_controller
{
    ...
    protected function private_core()
    {
        ...
        $this->auditoria = new auditoriaprecioarticulo();
        $this->auditoria->idarticulo = $this->articulo->referencia;
    ...
    {
        if($dato=="TRUE"){
            $sql = "UPDATE articulos SET servicio='1' WHERE referencia='".$this->articulo->referencia."'";
    ...
    private function edit_precio()
    {
        ...
        if (isset($_POST['preciocoste'])) {
            ...
            $this->articulo->preciocombo = floatval($_POST['preciocombo']);
        }

        if ($this->articulo->save()) {

            $auditar = new auditoriaprecioarticulo();
            $auditar->costo =  $this->articulo->preciocoste ;
            $auditar->preciocombo =  $this->articulo->preciocombo;
            ...
            $auditar->precio = $this->articulo->pvp;
            $auditar->precioanterior = $_POST['precioanterior'];
            $auditar->idarticulo = $this->articulo->referencia;
    ...
    private function modificar_articulo()
    {
        ...
        $this->articulo->servicio = isset($_POST['servicio']);
        $this->articulo->por_liquidacion = $_POST['por_liquidacion'];
        $this->articulo->necesita_articulo = $_POST['necesita_articulo'];
