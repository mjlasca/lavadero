 ...
 */
class registro_gasto extends \fs_model
{
    ...
    {
        parent::__construct('registro_gastos');
        if ($data) {
    ...
    public function url()
    {
        if (is_null($this->idfactura)) {
            return 'index.php?page=registros_gastos';
        }

        return 'index.php?page=registros_gasto&id=' . $this->idfactura;
    }

    /**
     * Devuelve las líneas de la factura.
     * @return line_registro_gasto
     */
     ...
     * Si no hay, las crea.
     * @return \linea_iva_registro_gasto
     */
    public function get_lineas_iva()
    {
        return $this->get_lineas_iva_trait('\linea_iva_registro_gastos');
    }
    ...
    public function get_rectificativas()
    {
        ...
        if ($data) {
            foreach ($data as $d) {
                $devoluciones[] = new \registro_gasto($d);
            }
        }
    ...
    public function get($id)
    {
        ...
        if ($data) {
            return new \registro_gasto($data[0]);
        }
    ...
    public function full_test($duplicados = TRUE)
    {
        ...
        $this->get_lineas_iva();
        $linea_iva = new \linea_iva_registro_gastos();
        ...
        if ($status && $duplicados) {
            ...
            if ($facturas) {
                foreach ($facturas as $fac) {
                    ...
                    $aux = $this->db->select("SELECT referencia FROM lineasregistro_gastos WHERE
                  idfactura = " . $this->var2str($this->idfactura) . "
                  AND referencia NOT IN (SELECT referencia FROM lineasregistro_gastos
                  WHERE idfactura = " . $this->var2str($fac['idfactura']) . ");");
                    if (!$aux) {
                        $this->new_error_msg("Esta factura es un posible duplicado de
                     <a href='index.php?page=registros_gasto&id=" . $fac['idfactura'] . "'>esta otra</a>.
