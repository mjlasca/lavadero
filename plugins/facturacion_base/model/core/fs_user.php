...
class fs_user extends \fs_model
{
    ...
    public function __construct($data = FALSE)
    {
        ...
        if ($data) {
            ...
            $this->user_admin = $this->str2bool($data['user_admin']);
            $this->user_jefe_patio = $this->str2bool($data['user_jefe_patio']);
    ...
    /*
     * Función para generar el perfil del usuario user_admin
     * @return boolean
     */
    public function perfil_user_admin()
    {
        $access = new fs_access();
        
        
        $access->fs_page = $p->fs_page;
        $access->fs_user = $nu->nick;
        $access->allow_delete = $p->allow_delete;
        $access->save();
    }
    
    
    /*
     * Función para generar el perfil del usuario jefe de patio
     * @return boolean
     */
    public function perfil_user_jefe(){
        ...
        if ($this->test()) {
            ...
            if ($this->exists()) {
                $sql = "UPDATE " . $this->table_name . " SET password = " . $this->var2str($this->password)
                    ...
                    . ", user_admin = " . $this->var2str($this->user_admin)
                    . ", user_jefe_patio = " . $this->var2str($this->user_jefe_patio)
               ...
               last_login,last_login_time,last_ip,last_browser,fs_page,css,user_jefe_patio,user_admin) VALUES
               (" . $this->var2str($this->nick)
                    ...
                    . "," . $this->var2str($this->user_jefe_patio)
                    . "," . $this->var2str($this->user_admin) . ");";
            }
