...
class gasto extends \fs_model
{
    ...
    protected function install()
    {
        ...
        /**
         * La tabla gastos tiene varias claves ajenas, por eso debemos forzar la comprobación de esas tablas.
         */
        new \fabricante();
        new \familia();
        new \impuesto();
    ...
    public function url()
    {
        if (is_null($this->referencia)) {
            return "index.php?page=ventas_gastos";
        }

        return "index.php?page=ventas_gasto&ref=" . urlencode($this->referencia);
    }
    ...
    public function get_stock()
    {
        ...
        $stock = new \stock();
        return $stock->all_from_gasto($this->referencia);
    }
    ...
    public function get_lineas_albaran_cli($offset = 0, $limit = FS_ITEM_LIMIT)
    {
        ...
        return $linea->all_from_gasto($this->referencia, $offset, $limit);
    }
    ...
    {
        if (file_exists(FS_MYDOCS . 'images/gastos/' . $this->image_ref() . '-1.png')) {
            return 'images/gastos/' . $this->image_ref() . '-1.png';
        } else if (file_exists(FS_MYDOCS . 'images/gastos/' . $this->image_ref() . '-1.jpg')) {
            return 'images/gastos/' . $this->image_ref() . '-1.jpg';
        }
    ...
    public function set_imagen($img, $png = TRUE)
    {
        ...
        if (file_exists(FS_MYDOCS . 'images/gastos/' . $this->image_ref() . '-1.png')) {
            unlink(FS_MYDOCS . 'images/gastos/' . $this->image_ref() . '-1.png');
        } else if (file_exists('images/gastos/' . $this->image_ref() . '-1.jpg')) {
            unlink(FS_MYDOCS . 'images/gastos/' . $this->image_ref() . '-1.jpg');
        }

        if ($img) {
            if (!file_exists(FS_MYDOCS . 'images/gastos')) {
                @mkdir(FS_MYDOCS . 'images/gastos', 0777, TRUE);
            }

            if ($png) {
                $f = @fopen(FS_MYDOCS . 'images/gastos/' . $this->image_ref() . '-1.png', 'a');
            } else {
                $f = @fopen(FS_MYDOCS . 'images/gastos/' . $this->image_ref() . '-1.jpg', 'a');
            }
    ...
    public function set_referencia($ref)
    {
        ...
        if (is_null($ref) || strlen($ref) < 1 || strlen($ref) > 18) {
            ...
            if ($this->db->exec($sql)) {
                ...
                if (file_exists(FS_MYDOCS . 'images/gastos/' . $this->image_ref() . '-1.png')) {
                    rename(FS_MYDOCS . 'images/gastos/' . $this->image_ref() . '-1.png', FS_MYDOCS . 'images/gastos/' . $this->image_ref($ref) . '-1.png');
                }
    ...
    public function set_stock($codalmacen, $cantidad = 1)
    {
        ...
        if ($this->nostock) {
            ...
            $stock = new \stock();
            $encontrado = FALSE;
            $stocks = $stock->all_from_gasto($this->referencia);
            ...
            if ($result) {
                ...
                $nuevo_stock = $stock->total_from_gasto($this->referencia);
    ...
    public function delete()
    {
        ...
        $sql = "DELETE FROM gastosprov WHERE referencia = " . $this->var2str($this->referencia) . ";";
    ...
    private function new_search_tag($tag)
    {
        ...
        if (strlen($tag) > 1) {
            ...
            if ($actualizar) {
                $this->cache->set('gastos_searches', self::$search_tags, 5400);
            }
        }
    ...
    public function get_search_tags()
    {
        if (!isset(self::$search_tags)) {
            self::$search_tags = $this->cache->get_array('gastos_searches');
        }
        ...
        if (!self::$cleaned_cache) {
            ...
            if (!empty(self::$search_tags)) {
                foreach (self::$search_tags as $value) {
                    $this->cache->delete('gastos_search_' . $value['tag']);
                }
            }
    ...
    public function search($query = '', $offset = 0, $codfamilia = '', $con_stock = FALSE, $codfabricante = '', $bloqueados = FALSE)
    {
        ...
        if ($query != '' && $offset == 0 && $codfamilia == '' && $codfabricante == '' && ! $con_stock && ! $bloqueados) {
            ...
            if ($this->new_search_tag($query)) {
                $artilist = $this->cache->get_array('gastos_search_' . $query);
            }
        }
        ...
        if ($this->get_search_tags()) {
            foreach (self::$search_tags as $i => $value) {
                if ($value['expires'] < time()) {
                    ...
                    $this->cache->set('gastos_search_' . $value['tag'], $this->search($value['tag']), 5400);
                    echo '.';
                }
            }
