...
class articulo extends \fs_model
{
    ...
    /**
     * Se cambia a servicio
     * @var int
     */
    public $servicio;
    
    /**
     * valor del artículo en combo
     * @var double
     */
    public $preciocombo;
	
	/**
     * Indica si es un combo el artíuclo
     * @var int
     */
    public $es_combo;
    
    /**
     * Porcentaje de comisión para los artículos a la hora de facturación y 
     * @var double
     */
    public $por_liquidacion;
    
    /**
     * Valor para solicitar los artículos
     * @var double
     */
    public $necesita_articulo;
    
    /**
     * Valor de registro articulo
     * @var double
     */
    public $id_articulo;
    
    /**
     * Código de estado para desactivar
     * @var int
     */
    public $cod_estado;
    ...
    public function __construct($data = FALSE)
    {
        ...
        if (!isset(self::$impuestos)) {
            ...
            self::$column_list = 'referencia,codfamilia,codfabricante,descripcion,pvp,factualizado,costemedio,' .
                ...
                'partnumber,codsubcuentacom,codsubcuentairpfcom,trazabilidad,servicio,preciocombo,es_combo,por_liquidacion,necesita_articulo,id_articulo,cod_estado';
        }
        ...
        if ($data) {
            ...
            $this->servicio = $this->intval($data['servicio']);
            $this->preciocombo = floatval($data['preciocombo']);
            $this->es_combo = floatval($data['es_combo']);
            $this->por_liquidacion = floatval($data['por_liquidacion']);
            $this->necesita_articulo = floatval($data['necesita_articulo']);
            $this->id_articulo = floatval($data['id_articulo']);
            $this->cod_estado = $data['cod_estado'];
            $this->imagen = NULL;
            $this->exists = TRUE;
        } else {
            ...
            $this->preciocombo = 0;
            $this->es_combo = 0;
            $this->por_liquidacion = 0;
            $this->necesita_articulo = 0;
            $this->id_articulo = 0;
            $this->cod_estado = 0;
            $this->imagen = NULL;
            $this->exists = FALSE;
    ...
    {
        $data = $this->db->select("SELECT " . self::$column_list . " FROM " . $this->table_name . " WHERE cod_estado=1 AND referencia = " . $this->var2str($ref) . ";");
        if ($data) {
    ...
    public function get_equivalentes()
    {
        ...
        if (isset($this->equivalencia)) {
            $data = $this->db->select("SELECT " . self::$column_list . " FROM " . $this->table_name .
                " WHERE cod_estado=1 AND equivalencia = " . $this->var2str($this->equivalencia) . " ORDER BY referencia ASC;");
            if ($data) {
                foreach ($data as $d) {
        ...
        if ($this->test()) {
            ...
            if ($this->exists()) {
                $sql = "UPDATE " . $this->table_name . " SET descripcion = " . $this->var2str($this->descripcion) .
                    ...
                    ", servicio = " . $this->var2str($this->servicio) .
                    ", preciocombo = " . $this->var2str($this->preciocombo) .
                    ", es_combo = " . $this->var2str($this->es_combo) .
                    ", por_liquidacion = " . $this->var2str($this->por_liquidacion) .
                    ", necesita_articulo = " . $this->var2str($this->necesita_articulo) .
                    "  WHERE referencia = " . $this->var2str($this->referencia) . ";";
                ...
                $sql = "INSERT INTO " . $this->table_name . " (" . self::$column_list . ") VALUES (" .
                    ...
                    $this->var2str($this->servicio) . "," .
                    $this->var2str($this->preciocombo) . "," .
                    $this->var2str($this->es_combo) . "," .
                    $this->var2str($this->por_liquidacion) . "," .
                    $this->var2str($this->necesita_articulo) . "," .
                    $this->var2str($this->id_articulo) . ", '1');";
            }
    ...
    public function search($query = '', $offset = 0, $codfamilia = '', $con_stock = FALSE, $codfabricante = '', $bloqueados = FALSE)
    {
        ...
        if (count($artilist) <= 1) {
            ...
            $separador = ' WHERE cod_estado=1 AND ';
