...
function calcular_precio_venta(linea){
    if(parseFloat( $("#porutil_"+linea).val() ) >= 0){
        var total_aUtil = parseFloat( $("#total_"+linea).val() ) / parseFloat( $("#cantidad_"+linea).val() ) ;
        var resultadito = Math.round((total_aUtil * ((parseFloat( $("#porutil_"+linea).val()) / 100)+1))); 
...
function add_articulo(ref, desc, pvp, dto, codimpuesto, cantidad, prec_combo, prec_venta, codcombinacion)
{
    ...
    if (typeof prec_combo == 'undefined') {
        prec_combo = 0;
    }
    ...
    <td><input type=\"text\" class=\"form-control text-right\" id=\"pvta" + numlineas + "\" name=\"pvta" + numlineas + "\" value=\"" + prec_venta +
    "\" onkeyup=\"recalcular()\" onclick=\"this.select()\" autocomplete=\"off\"/></td>\n\
      <td><input type=\"text\" class=\"form-control text-right\" id=\"pcombo" + numlineas + "\" name=\"pcombo" + numlineas + "\" value=\"" + prec_combo +
            "\" onkeyup=\"recalcular()\" onclick=\"this.select()\" autocomplete=\"off\"/></td>\n\
      ...
      <td><input type=\"text\" class=\"form-control text-right\" id=\"porutil_" + numlineas + "\" name=\"porutil_" + numlineas + "\" value=\"" + utilidad_general +
            "\" onkeyup=\"calcular_precio_venta("+numlineas+")\" onclick=\"this.select()\" autocomplete=\"off\"/></td>\n\
...
function get_precio_combo(referencia_bus)
{
  var precCombo = 0;
    if (nueva_compra_url !== '') {
        $.getJSON(nueva_compra_url, 'referencia_combo=' + referencia_bus, function (json) {
				precCombo = json[0].preciocombo;
        });
    }
                ...
                $.each(json, function (key, val) {
                    ...
                    if (val.secompra) {
                        var funcion = "add_articulo('" + val.referencia + "','" + descripcion + "','" + precio + "','"
                                + val.dtopor + "','" + val.codimpuesto + "','" + val.cantidad + "','" + val.preciocombo + "','" + val.pvp + "')";
