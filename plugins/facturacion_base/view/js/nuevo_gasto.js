...
function recalcular() {
  ...
  for (var i = 0; i < numlineas; i++) {
    if ($("#linea_" + i).length > 0) {
      ...
      if (numlineas == 1) {
        $("#total_" + i).val(
          fs_round(l_neto, fs_nf0) +
            fs_round((l_neto * (l_iva - l_irpf + l_recargo)) / 100, fs_nf0)
        );
      } else {
        $("#total_" + i).val(
          number_format(
            l_neto + (l_neto * (l_iva - l_irpf + l_recargo)) / 100,
            fs_nf0,
            ".",
            ""
          )
      ...
      //Se calcula el precio de venta de acuerdo al porcentaje de utilidad
      calcular_precio_venta(i);
    }
  }
...
function ajustar_total(i) {
  ...
  if ($("#linea_" + i).length > 0) {
    ...
    if (
      l_total <=
      l_pvp * l_uds + (l_pvp * l_uds * (l_iva - l_irpf + l_recargo)) / 100
    ) {
...
function aux_all_impuestos(num, codimpuesto) {
  ...
  var html =
    '<td><select id="iva_' +
    num +
    '" class="form-control" name="iva_' +
    num +
    '" onchange="ajustar_iva(\'' +
    num +
    "')\">";
  for (var i = 0; i < all_impuestos.length; i++) {
  ...
  html +=
    '<td class="recargo"><input type="text" class="form-control text-right" id="recargo_' +
    num +
    '" name="recargo_' +
    num +
    '" value="' +
    recargo +
    '" onclick="this.select()" onkeyup="recalcular()" autocomplete="off"/></td>';

  html +=
    '<td class="irpf"><input type="text" class="form-control text-right" id="irpf_' +
    num +
    '" name="irpf_' +
    num +
    '" value="' +
...
function calcular_precio_venta(linea) {
  if (parseFloat($("#porutil_" + linea).val()) >= 0) {
    var total_aUtil =
      parseFloat($("#total_" + linea).val()) /
      parseFloat($("#cantidad_" + linea).val());
    var resultadito = Math.round(
      total_aUtil * (parseFloat($("#porutil_" + linea).val()) / 100 + 1)
    );
    ...
    $("#pvta" + linea).val(resultadito);
    utilidad_general = parseFloat($("#porutil_" + linea).val());
  }
}
...
function add_articulo(
  ...
  if (typeof prec_venta == "undefined") {
    prec_venta = 0;
  }
...
<tr id="linea_${numlineas}">
    ...
    <td><input type="${input_number}" step="any" id="cantidad_${numlineas}" class="form-control text-right" name="cantidad_${numlineas}" value="${cantidad}" onchange="recalcular()" onkeyup="recalcular()" autocomplete="off"/></td>
    <td><button class="btn btn-sm btn-danger" type="button" onclick="$('#linea_${numlineas}').remove();recalcular();"><span class="glyphicon glyphicon-trash"></span></button></td>
    <td><input type="text" class="form-control text-right" id="pvp_${numlineas}" name="pvp_${numlineas}" value="${pvp}" onkeyup="recalcular()" onclick="this.select()" autocomplete="off"/></td>
    <td><input type="text" id="dto_${numlineas}" name="dto_${numlineas}" value="${dto}" class="form-control text-right" onkeyup="recalcular()" onchange="recalcular()" onclick="this.select()" autocomplete="off"/></td>
    <td><input type="text" class="form-control text-right" id="neto_${numlineas}" name="neto_${numlineas}" onchange="ajustar_neto(${numlineas})" onclick="this.select()" autocomplete="off"/></td>
    ${aux_all_impuestos(numlineas, codimpuesto)}
    <td><input type="text" class="form-control text-right" id="porutil_${numlineas}" name="porutil_${numlineas}" value="${utilidad_general}" onkeyup="calcular_precio_venta(${numlineas})" onclick="this.select()" autocomplete="off"/></td>
    <td class="warning" title="Cálculo aproximado del total de la linea">
        <input type="text" class="form-control text-right" id="total_${numlineas}" name="total_${numlineas}" onchange="ajustar_total(${numlineas})" onclick="this.select()" autocomplete="off"/>
    </td>
</tr>`;
  $("#lineas_doc").append(filaHTML);
  numlineas += 1;
...
function add_linea_libre() {
  $("#lineas_doc").append(
    '<tr id="linea_' +
      numlineas +
      '">\n\
      <td><input type="hidden" name="idlinea_' +
      numlineas +
      '" value="-1"/>\n\
         <input type="hidden" name="referencia_' +
      numlineas +
      '"/>\n\
         <input type="hidden" name="codcombinacion_' +
      numlineas +
      '"/>\n\
         <div class="form-control"></div></td>\n\
      <td><textarea class="form-control" id="desc_' +
      numlineas +
      '" name="desc_' +
      numlineas +
      '" rows="1"></textarea></td>\n\
      <td><input type="' +
      input_number +
      '" step="any" id="cantidad_' +
      numlineas +
      '" class="form-control text-right" name="cantidad_' +
      numlineas +
      '" onchange="recalcular()" onkeyup="recalcular()" autocomplete="off" value="1"/></td>\n\
      <td><button class="btn btn-sm btn-danger" type="button" onclick="$(\'#linea_' +
      numlineas +
      '\').remove();recalcular();">\n\
         <span class="glyphicon glyphicon-trash"></span></button></td>\n\
      <td><input type="text" class="form-control text-right" id="pvp_' +
      numlineas +
      '" name="pvp_' +
      numlineas +
      '" value="0"\n\
          onkeyup="recalcular()" onclick="this.select()" autocomplete="off"/></td>\n\
      <td><input type="text" id="dto_' +
      numlineas +
      '" name="dto_' +
      numlineas +
      '" value="0" class="form-control text-right"\n\
          onkeyup="recalcular()" onclick="this.select()" autocomplete="off"/></td>\n\
      <td><input type="text" class="form-control text-right" id="neto_' +
      numlineas +
      '" name="neto_' +
      numlineas +
      '" onchange="ajustar_neto(' +
      numlineas +
      ')" onclick="this.select()" autocomplete="off"/></td>\n\
      ' +
      aux_all_impuestos(numlineas, default_impuesto) +
      '\n\
      <td class="warning" title="Cálculo aproximado del total de la linea">\n\
         <input type="text" class="form-control text-right" id="total_' +
      numlineas +
      '" name="total_' +
      numlineas +
      '" onchange="ajustar_total(' +
      numlineas +
      ')" onclick="this.select()" autocomplete="off"/></td></tr>'
  );
  numlineas += 1;
  $("#numlineas").val(numlineas);
  recalcular();
      ...
      success: function (datos) {
        if (typeof datos[0] == "undefined") {
          ...
          document.f_buscar_articulos.query.value =
            document.f_nuevo_articulo.referencia.value;
...
function buscar_articulos() {
  document.f_nuevo_articulo.referencia.value =
    document.f_buscar_articulos.query.value;
  document.f_nuevo_articulo.refproveedor.value =
    document.f_buscar_articulos.query.value;
  ...
  if (document.f_buscar_articulos.query.value == "") {
    ...
    if (nueva_compra_url !== "") {
      ...
      $.getJSON(
        nueva_compra_url,
        $("form[name=f_buscar_articulos]").serialize(),
        function (json) {
          var items = [];
          var insertar = false;
          $.each(json, function (key, val) {
            ...
            if (val.nostock) {
              ...
              stock +=
                ' <span title="stock general">(' + val.stockfis + ")</span>";
            }
            ...
            if (val.codfamilia) {
              descripcion_visible +=
                ' <span class="label label-default" title="Familia: ' +
                val.codfamilia +
                '">' +
                val.codfamilia +
                "</span>";
            ...
            if (val.codfabricante) {
              descripcion_visible +=
                ' <span class="label label-default" title="Fabricante: ' +
                val.codfabricante +
                '">' +
                val.codfabricante +
                "</span>";
            ...
            items.push(
              tr_aux +
                ...
                funcion1 +
                '">' +
                show_precio(val.coste, val.coddivisa) +
                '</a></td>\n\
                     <td class="text-right"><a href="#" onclick="return ' +
                funcion2 +
                '" title="actualizado el ' +
          ...
          if (items.length == 0 && !fin_busqueda1) {
            items.push(
              '<tr><td colspan="4" class="warning">Sin resultados. Usa la pestaña\n\
