...
function verificarVarios(){
    result  = true;

    dato = parseFloat($("#por_liquidacion").val());
    //alert("HOLA MUNDO " +dato);
    
    if(dato<0 || dato > 100){
        result =  false;
    }
    
    dato = parseFloat($("#necesita_articulo").val());
    //alert("HOLA MUNDO " +dato);
    ...
    if(!result){
        bootbox.alert("Revise porcentajes de liquidación");
        return result;
    }
...
$(document).ready(function () {
    ...
    $("#formArticulos").submit(function (event) {
        verificarVarios();
    });
