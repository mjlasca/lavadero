...
<script type="text/javascript">
   ...
   {
      $('#ul_tabs a[href="#filtros"]').tab('show');
   }
   ...
   $(document).ready(function() {
      ...
      else if(window.location.hash.substring(1) == 'filtros')
      {
         show_filtros();
      }
      else
...
<div role="tabpanel">
   <ul id="ul_tabs" class="nav nav-tabs" role="tablist">
      ...
      <li role="presentation">
         <a href="#filtros" aria-controls="filtros" role="tab" data-toggle="tab">
   ...
   <div class="tab-content">
      ...
      <div role="tabpanel" class="tab-pane" id="filtros">
         <br/>
         <form name="f_nueva_tarifa" action="<?php echo $fsc->url();?>#filtros" method="post" class="form">
            <input type="hidden" name="form_filtros" value="true">
            <div class="container-fluid">
               <div class="row">
                  <div class="col-sm-3">
                     <div class="form-group">
                        <div class="input-group">
                           <input class="form-control" type="text" name="query" value="<?php echo $fsc->filtro_b;?>" autocomplete="off" placeholder="Referencia o desc factura">
                           <span class="input-group-btn">
                              <button class="btn btn-primary hidden-sm" type="submit">
                                 <span class="glyphicon glyphicon-search"></span>
                              </button>
                  ...
                  <div class="col-sm-2">
                     <div class="form-group">
                        <select  class="form-control"  name="info_provee"  id="info_provee" onchange="this.form.submit()" >
                           <option value="" <?php if( $fsc->info_provee=='' ){ ?>selected="selected" <?php } ?>>Todos los lavadores</option>
                  ...
                  <div class="col-sm-3">
                     <div class="radio-inline">
                        <label>
                           <input type="radio" name="consolidado" value="0"  <?php if( $fsc->consolidado == '0'  ){ ?>checked="" <?php } ?> onchange="this.form.submit()">
                           Vista general
                        </label>
                     </div>
                     <div class="radio-inline">
                        <label>
                           <input type="radio" name="consolidado" value="1" <?php if( $fsc->consolidado == '1' ){ ?>checked="" <?php } ?> onchange="this.form.submit()">
                           Vista consolidada
                        </label>
                     </div>
                  </div>
                  <div class="col-sm-6">
                     <div class="radio-inline">
                        <label>
                           <input type="radio" name="estado" value=""  <?php if( $fsc->estado_factura == ''  ){ ?>checked="" <?php } ?> onchange="this.form.submit()">
                           Todas las facturas
                        </label>
                     </div>
                     <div class="radio-inline">
                        <label>
                           <input type="radio" name="estado" value="pagada" <?php if( $fsc->estado_factura == 'pagada' ){ ?>checked="" <?php } ?> onchange="this.form.submit()">
                           Las Pagadas
                        </label>
                     </div>
                     <div class="radio-inline">
                        <label>
                           <input type="radio" name="estado" value="pagada0" <?php if( $fsc->estado_factura == 'pagada0'  ){ ?>checked="" <?php } ?> onchange="this.form.submit()">
                           Sin pagar
                        </label>
                     </div>
                     <div class="radio-inline">
                        <label>
                           <input type="radio" name="estado" value="anulada" <?php if( $fsc->estado_factura == 'anulada'  ){ ?>checked="" <?php } ?> onchange="this.form.submit()">
                           Anuladas
                        </label>
                     </div>
                  </div>
         ...
         <table class="table table-hover">
            <thead>
               <tr>
                  ...
                  <th class="text-left">Código Factura</th>
                  <th class="text-left">Agente</th>
                  <th class="text-left">Cliente</th>
                  <th class="text-left">Referencia</th>
                  <th class="text-left">Descripción</th>
                  <th class="text-center">Familia</th>
                  <th class="text-left">Prov. Servicio</th>
                  <th class="text-center">Cantidad</th>
                  <th class="text-center">Total</th>
               </tr>
            </thead>
            <?php $loop_var1=$fsc->consulta_articulos; $counter1=-1; if($loop_var1) foreach( $loop_var1 as $key1 => $value1 ){ $counter1++; ?>

            <tr>
               <td class="text-left"><?php echo $value1["fecha"];?></td>
               <td class="text-left"><a href="index.php?page=ventas_factura&id=<?php echo $value1['idfactura'];?>"><?php echo $value1["codigo"];?></a></td>
               <td class="text-left"><?php echo $value1["codagente"];?></td>
               <td class="text-left"><?php echo $value1["nombrecliente"];?></td>
               <td class="text-left"><?php echo $value1["referencia"];?></td>
               <td class="text-left"><?php echo $value1["descripcion"];?></td>
               <td class="text-center"><?php echo $value1["codfamilia"];?></td>
               <td class="text-left"><?php echo $value1["proveedor_servicio"];?></td>
               <td class="text-right"><?php echo $value1["cantidad"];?></td>
               <td class="text-right"><?php echo $fsc->show_precio($value1["pvptotal"]);?></td>
            </tr>
            <?php } ?>

            <?php if( $fsc->total_articulos ){ ?>

               <tr class="btn-default">
                  <th class="hidden-sm"></th>
                  <td colspan="8" class="text-right">
                     <b>CANT. TOTAL -> <?php echo $fsc->total_articulos["cantidad"];?></b>
                  </td>
                  <td colspan="8" class="text-right">
                     <b>TOTAL -> <?php echo $fsc->show_precio($fsc->total_articulos["total"]);?></b>
                  </td>
               </tr>
            <?php }else{ ?>
               ...
               <tr class="warning">
                  <td></td>
                  <td></td>
                  <td colspan="7">Ningún resultado encontrado</td>
                  <th class="hidden-sm"></th>
